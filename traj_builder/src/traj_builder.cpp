...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose, 
        ...
        nav_msgs::Odometry state = vec_of_states.front();
        geometry_msgs::Point pos = state.pose.pose.position;
		vec_of_states.clear();

		double linearV = state.twist.twist.linear.x;
		double angularV = state.twist.twist.angular.z;
		double linearVmax = accel_max_ * dt_;
		double angularVmax = alpha_max_ * dt_;

		if (linearV != 0 || angularV != 0){
			for (double i = 0; i < angularV; i++){

				angularV -= angularVmax;
				state.twist.twist.angular.z = angularV;
				vec_of_states.push_back(state);
			}
			
			for (double i = 0; i < linearV; i++){
				
				linearV -= linearVmax;
				state.twist.twist.linear.x = linearV;
				vec_of_states.push_back(state);
			}
		}
		state.twist.twist = halt_twist_;
		vec_of_states.push_back(state);
